...
module tb_farm;
    ...
    integer len_hist [0:16];                  // count of names per length (0..16)
    integer len_distinct;                     // # of distinct name-lengths observed
    ...
    task clear_run_state;
        ...
        begin
            ...
            for (a = 0; a <= 16; a = a + 1) len_hist[a] = 0;
        end
    endtask
    ...
    always @(posedge clk) begin
        ...
        if (run_en && !rec_empty && !rec_rd_en && drain_go) begin
            ...
            // name-length histogram (length-distribution sanity in sampled mode)
            if (r_len <= 8'd16) len_hist[r_len] = len_hist[r_len] + 1;
    ...
    initial begin
        ...
        if (sample_mode) begin
            ...
            // name-length spread: sampling must not collapse to a single length, and every
            // observed length must be in the legal 1..16 band (len 0 or >16 = corrupt record).
            len_distinct = 0;
            for (i = 0; i <= 16; i = i + 1) begin
                if (len_hist[i] > 0) begin
                    len_distinct = len_distinct + 1;
                    if (i < 1 || i > 16) begin
                        $display("FARM FAIL: %0d names with illegal length %0d (outside 1..16)", len_hist[i], i);
                        $finish;
                    end
                end
            end
            if (len_distinct < 3) begin
                $display("FARM FAIL: only %0d distinct name-lengths over %0d names -- length distribution collapsed",
                         len_distinct, got);
                $finish;
            end
            $display("distribution: pos0 distinct first-letters=%0d, distinct lengths=%0d, names differing from first=%0d",
                     pos_distinct[0], len_distinct, n_diff);
